...
loop:	lodd myCounter:			; Load  counter of numbers to Accumulator (ac)
		...
		subd constant1:			; decrement 1 from myCounter (the accumuator of value counter - cons1 .  => counter - cons1)
		stod myCounter:			; store the result from substraction to counter . => count = count - cons1)
one:	lodd daddr: 			; load poiter array to accumuator (ac))
		pshi					; push argument to stack
		addd constant1:			; Increment 1
		stod daddr:				; Store pointer array for next fib 
		call fib:				; Call fib with the arg that is on stack
		insp 1					; Clear stack 
two:	push					; Put return on stack (put the result from f(n) from ac to stack )
...
fib:	lodl 1					; Load the arg from stack
		...
		subd constant1:			; decrement argument (f(n - 1))
		...
		subd constant1: 		; ac is arg - 2 (f(n - 2))
